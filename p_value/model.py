...
class ELM(object):
  def __init__(self, sess, batch_size, input_len, hidden_num, output_len):
    ...
    # self._correct_prediction = tf.equal(int(self._fx1), self._t1)
    # print "Y",self._t1
    # print "predict",self._fx1
  ...
  def test(self, x, t=None):
    ...
    if t is not None :
      ...
      return self._sess.run(self._accuracy, {self._x1:x, self._t1:t})
      # print "Y", self._sess.run(self._t1,  {self._x1:x, self._t1:t})
      # print "predict", self._sess.run(self._fx1,  {self._x1:x, self._t1:t})
    else :
