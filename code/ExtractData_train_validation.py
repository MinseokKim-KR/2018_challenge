...
def get_files(drivers=None, PATH=None):
...
def make_x(table): # make input data to nomalization and reshape 300 * 2
    MAX = [0, 0, 0]
    MIN = [0, 0, 0] #set up the max and min of the two sensor
    x_data=[]
...
def extract_data_oned(drivers=None, labels=None, numRows=None, data_types = None, numData=None, mode=None, NUM_CHANNELS=None,
    ...
    while numDataCount < numData:
        ...
        if mode is 'train':
            row_idx = random.randint(0, int(max_row_idx * 4 / 5))  ## train : 4 / 5
        elif mode is 'validate':
            row_idx = random.randint(int(max_row_idx * 4 / 5), max_row_idx)  ## validate : 1 / 5
        if ONED:
        ...
        print('numDataCount : ', numDataCount)
